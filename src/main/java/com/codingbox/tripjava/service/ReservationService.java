...
public class ReservationService {
	...
	public List<CancelReservationListDTO> getCancelledReservationsByUserId(Integer userId) {
		...
		return reservations.stream()
							.map(reservation -> new CancelReservationListDTO(
											reservation.getPayment().getPaymentId(),
											reservation.getResvId(), 
											reservation.getUser().getUsername(),
											reservation.getRoomTypePrice().getAccommodation().getAccomName(),
											reservation.getRoomTypePrice().getRoomType(), 
											reservation.getNumberGuests(),
											new Date(reservation.getCheckInDate().getTime()),
											new Date(reservation.getCheckOutDate().getTime()), 
											reservation.getResvStatus()
									)
							)
							.collect(Collectors.toList());
	}
