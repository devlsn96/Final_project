...
public class UserService {
	...
	public User processKakaoUser(User kakaoUser) {
		if (kakaoUser.getEmail() == null || kakaoUser.getUsername() == null) {
			throw new IllegalArgumentException("사용자 정보가 누락되었습니다. (email: " + kakaoUser.getEmail() + ", username: "
					+ kakaoUser.getUsername() + ")");
		}
		// 1. 기존 회원 검사
		if (userRepository.existByEmail(kakaoUser.getEmail()) != null) {
			return kakaoUser;
		}
	...
	// 비밀번호 재설정
	// @Transactional
	// public User updatePassword(String email, User user) throws Exception {
